...
⚠️ **快捷鍵開完一定要 `app:activate()`。** 面板是 utility 樣式的視窗（NSPanel），
這種視窗在擁有它的 App 不是前景時會被系統**直接隱藏**。快捷鍵一定是從別的 App
按下去的，所以不 activate 的話：面板閃一下就不見，或看得到卻打不了字。
實測：`show()` 之後視窗數 1 → 一失焦就變 0；補上 `activate()` 之後穩定留著。
同一個原因，**點回別的視窗時面板會自動收起來**——那是系統行為，不是壞掉。
...
**學起來會同步進詞彙表**（2026-09-18，Spark 端 `learned_vocab_add`）：存校正規則時，
「對的寫法」也加進這個人的詞彙表，面板 TAG 看得到，whisper 下次一開始就聽對。
含標點的片語（「中吧。林玉娟」）不加，只佔提示詞預算。加詞失敗不影響校正規則已存好。

⚠️ **系統分不出「改錯字」和「改用字」**。實測有人把「但是」改成「不過」、點了學起來，
結果之後每句「但是」都被換掉。泡泡出來時要看清楚再點；存錯了去 Spark 的
`~/.config/voice-input/corrections.<email>.tsv` 刪那一行（`.counts.json` 同名的鍵也刪）。

面板也跟著整理：靈敏度（這台 Mac 的門檻，留白＝跟著 Spark 全域值，框裡灰字是目前實際用的數字）
搬到音量條正下方；GATE／LOG／其他設定收進 MORE 折疊區。RESULT 卡片（複製、重新貼上、手動學起來、
開始錄音）整個拿掉——學起來已經自動了，其他的熱鍵和浮動圖示都做得到。
